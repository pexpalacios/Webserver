	...
	while (SignalHandler::running == 1)
	{
		...
		while (j < _pollFd.size())
		{
			...
			if (_pollFd[j].revents & POLLIN)
			{
				...
				{
					std::cout << "new connection on " << fd << " and server" << owner->getStaticRoot() << std::endl;
					handleNewConnection(fd, *owner);
				} 
				else 
				{
					std::cout << "Client connection on " << fd << "and server" << owner->getStaticRoot() << std::endl;
					handleClientConnection(fd, *owner);
					_pollFd.erase(_pollFd.begin() + j);
					continue;
				}
			}
